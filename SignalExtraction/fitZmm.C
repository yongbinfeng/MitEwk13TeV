    ...
    )
{
    ...
    hZmm->Sumw2();
    TH1D* hZxx = new TH1D("hZxx", "", NBINS, MASS_LOW, MASS_HIGH);
    hZxx->Sumw2();
    TH1D* hWx = new TH1D("hWx", "", NBINS, MASS_LOW, MASS_HIGH);
    hWx->Sumw2();
    TH1D* hTtb = new TH1D("hTtb", "", NBINS, MASS_LOW, MASS_HIGH);
    hTtb->Sumw2();
    TH1D* hDib = new TH1D("hDib", "", NBINS, MASS_LOW, MASS_HIGH);
    hDib->Sumw2();
    ...
    for (int j = 0; j < nWeight; ++j) {
        ...
        sprintf(hname, "hData_%s", (vWeight[j]).c_str());
        hDataUnc[j] = new TH1D(hname, "", NBINS, MASS_LOW, MASS_HIGH);
        cout << hDataUnc[j]->GetName() << endl;
        sprintf(hname, "hZmm_%s", (vWeight[j]).c_str());
        hZmmUnc[j] = new TH1D(hname, "", NBINS, MASS_LOW, MASS_HIGH);
        sprintf(hname, "hZxx_%s", (vWeight[j]).c_str());
        hZxxUnc[j] = new TH1D(hname, "", NBINS, MASS_LOW, MASS_HIGH);
        sprintf(hname, "hWx_%s", (vWeight[j]).c_str());
        hWxUnc[j] = new TH1D(hname, "", NBINS, MASS_LOW, MASS_HIGH);
        sprintf(hname, "hTtb_%s", (vWeight[j]).c_str());
        hTtbUnc[j] = new TH1D(hname, "", NBINS, MASS_LOW, MASS_HIGH);
        sprintf(hname, "hDib_%s", (vWeight[j]).c_str());
        hDibUnc[j] = new TH1D(hname, "", NBINS, MASS_LOW, MASS_HIGH);
    }
    ...
    TH1D* hZmmUp = new TH1D("hZmmUp", "", NBINS, MASS_LOW, MASS_HIGH);
    hZmmUp->Sumw2();
    TH1D* hZmmDown = new TH1D("hZmmDown", "", NBINS, MASS_LOW, MASS_HIGH);
    hZmmDown->Sumw2();
    ...
    for (UInt_t ifile = 0; ifile < fnamev.size(); ifile++) {
        ...
        for (UInt_t ientry = 0; ientry < intree->GetEntries(); ientry++) {
            ...
            if (typev[ifile] == eData) {
                ...
                if (typev[ifile] == eZmm) {
                    if (genVMass < MASS_LOW || genVMass > MASS_HIGH)
                        yield_wm += weight * corr;
                    if (genVMass < MASS_LOW || genVMass > MASS_HIGH)
                        continue;
