    ...
    const TString input_section = "1")
{
    ...
    if (sqrts == "13TeV") {
        ...
        fnamev.push_back(inputDir + TString("/") + TString("wx_select.raw.root"));
        typev.push_back(eEWK);
        fnamev.push_back(inputDir + TString("/") + TString("zxx_select.raw.root"));
        typev.push_back(eEWK);
        fnamev.push_back(inputDir + TString("/") + TString("top_select.raw.root"));
        typev.push_back(eEWK);
        fnamev.push_back(inputDir + TString("/") + TString("ww_select.raw.root"));
        typev.push_back(eEWK);
        fnamev.push_back(inputDir + TString("/") + TString("wz_select.raw.root"));
    ...
    //
    TH1D* hDataMet = new TH1D("hDataMet", "", NBINS, 0, METMAX);
    hDataMet->Sumw2();
    TH1D* hDataMetm = new TH1D("hDataMetm", "", NBINS, 0, METMAX);
    hDataMetm->Sumw2();
    TH1D* hDataMetp = new TH1D("hDataMetp", "", NBINS, 0, METMAX);
    hDataMetp->Sumw2();
    TH1D* hWmunuMet = new TH1D("hWmunuMet", "", NBINS, 0, METMAX);
    hWmunuMet->Sumw2();
    TH1D* hWmunuMetp = new TH1D("hWmunuMetp", "", NBINS, 0, METMAX);
    hWmunuMetp->Sumw2();
    TH1D* hWmunuMetm = new TH1D("hWmunuMetm", "", NBINS, 0, METMAX);
    hWmunuMetm->Sumw2();
    TH1D* hEWKMet = new TH1D("hEWKMet", "", NBINS, 0, METMAX);
    hEWKMet->Sumw2();
    TH1D* hEWKMetp = new TH1D("hEWKMetp", "", NBINS, 0, METMAX);
    hEWKMetp->Sumw2();
    TH1D* hEWKMetm = new TH1D("hEWKMetm", "", NBINS, 0, METMAX);
    hEWKMetm->Sumw2();

    TH1D* hDataRecoilp = new TH1D("hDataRecoilp", "", NBINS, 0, METMAX);
    hDataRecoilp->Sumw2();
    TH1D* hWmunuRecoilp = new TH1D("hWmunuRecoilp", "", NBINS, 0, METMAX);
    hWmunuRecoilp->Sumw2();
    TH1D* hEWKRecoilp = new TH1D("hEWKRecoilp", "", NBINS, 0, METMAX);
    hEWKRecoilp->Sumw2();

    TH1D* hDataMassp = new TH1D("hDataMassp", "", 60, 60, 120);
    hDataMassp->Sumw2();
    TH1D* hWmunuMassp = new TH1D("hWmunuMassp", "", 60, 60, 120);
    hWmunuMassp->Sumw2();
    TH1D* hEWKMassp = new TH1D("hEWKMassp", "", 60, 60, 120);
    hEWKMassp->Sumw2();

    TH1D* hDataZpt = new TH1D("hDataZpt", "", NBINS, 0, METMAX);
    hDataZpt->Sumw2();
    TH1D* hWmunuZpt = new TH1D("hWmunuZpt", "", NBINS, 0, METMAX);
    hWmunuZpt->Sumw2();
    TH1D* hEWKZpt = new TH1D("hEWKZpt", "", NBINS, 0, METMAX);
    hEWKZpt->Sumw2();
    ...
    for (UInt_t ifile = 0; ifile < fnamev.size(); ifile++) {
        ...
        for (UInt_t ientry = 0; ientry < intree->GetEntries(); ientry++) {
            ...
            if (typev[ifile] == eData) {
                ...
                if (mu2.Pt() < PT_CUT)
                    continue;
                if ((mu1 + mu2).M() < MASS_LOW)
                    continue;
                if ((mu1 + mu2).M() > MASS_HIGH)
                    continue;
                ...
                if (q1 * q2 < 0) {
                    ...
                    hDataMetm->Fill(corrMetPhiLepton);
                }
            } else {
                TLorentzVector mu1;
                TLorentzVector mu2;
                ...
                if (typev[ifile] == eZmumu || typev[ifile] == eBKG) {
                    ...
                    if (q1 * q2 < 0) {
                        ...
                        for (int i = 0; i <= hh_diff->GetNbinsX(); ++i) {
                            if (vDilepCor.Mod() > hh_diff->GetBinLowEdge(i) && vDilepCor.Mod() < hh_diff->GetBinLowEdge(i + 1)) {
                                bin = i;
                                break;
                            }
                        }
    ...
    // Signal PDFs
    RooDataHist wmunuMet("wmunuMET", "wmunuMET", RooArgSet(pfmet), hWmunuMet);
    RooHistPdf pdfWm("wm", "wm", pfmet, wmunuMet, 1);
    RooDataHist wmunuMetp("wmunuMETp", "wmunuMETp", RooArgSet(pfmet), hWmunuMetp);
    RooHistPdf pdfWmp("wmp", "wmp", pfmet, wmunuMetp, 1);
    RooDataHist wmunuMetm("wmunuMETm", "wmunuMETm", RooArgSet(pfmet), hWmunuMetm);
    RooHistPdf pdfWmm("wmm", "wmm", pfmet, wmunuMetm, 1);

    RooDataHist wmunuRecoilp("wmunuRecoilp", "wmunuRecoilp", RooArgSet(pfmet), hWmunuRecoilp);
    RooHistPdf pdfWmpRecoil("wmprec", "wmprec", pfmet, wmunuRecoilp, 1);
    RooDataHist wmunuZpt("wmunuZpt", "wmunuZpt", RooArgSet(pfmet), hWmunuZpt);
    RooHistPdf pdfWmZpt("wmzpt", "wmzpt", pfmet, wmunuZpt, 1);
    RooDataHist wmunuMassp("wmunuMassp", "wmunuMassp", RooArgSet(pfmet2), hWmunuMassp);
    RooHistPdf pdfWmpMass("wmpmass", "wmpmass", pfmet2, wmunuMassp, 1);
    ...
    // EWK+top PDFs
    RooDataHist ewkMet("ewkMET", "ewkMET", RooArgSet(pfmet), hEWKMet);
    RooHistPdf pdfEWK("ewk", "ewk", pfmet, ewkMet, 1);
    RooDataHist ewkMetp("ewkMETp", "ewkMETp", RooArgSet(pfmet), hEWKMetp);
    RooHistPdf pdfEWKp("ewkp", "ewkp", pfmet, ewkMetp, 1);
    RooDataHist ewkMetm("ewkMETm", "ewkMETm", RooArgSet(pfmet), hEWKMetm);
    RooHistPdf pdfEWKm("ewkm", "ewkm", pfmet, ewkMetm, 1);

    RooDataHist ewkRecoilp("ewkRecoilp", "ewkRecoilp", RooArgSet(pfmet), hEWKRecoilp);
    RooHistPdf pdfEWKpRecoil("ewkprec", "ewkprec", pfmet, ewkRecoilp, 1);
    RooDataHist ewkZpt("ewkZpt", "ewkZpt", RooArgSet(pfmet), hEWKZpt);
    RooHistPdf pdfEWKZpt("pdfEWKZpt", "pdfEWKZpt", pfmet, ewkZpt, 1);
    RooDataHist ewkMassp("ewkMassp", "ewkMassp", RooArgSet(pfmet2), hEWKMassp);
    RooHistPdf pdfEWKpMass("ewkpmass", "ewkpmass", pfmet2, ewkMassp, 1);
    ...
    std::vector<double> CorrU1Sigma;
    std::vector<double> CorrU1SigmaErr;
    std::vector<double> CorrU1Sigma_MC;
    std::vector<double> CorrU1SigmaErr_MC;
    std::vector<double> CorrU1Sigma_MC_raw;
    std::vector<double> CorrU1SigmaErr_MC_raw;

    std::vector<double> CorrU2Sigma;
    std::vector<double> CorrU2SigmaErr;
    std::vector<double> CorrU2Sigma_MC;
    std::vector<double> CorrU2SigmaErr_MC;
    std::vector<double> CorrU2Sigma_MC_raw;
    std::vector<double> CorrU2SigmaErr_MC_raw;
    ...
    for (int i = 1; i <= hU2vsZpt->GetNbinsX(); i++) {
        ...
        mean = hU1vsZpt_rsp_pfy->GetMean();
        rms = hU1vsZpt_rsp_pfy->GetRMS();
        ...
        mean = hU1vsZpt_pfy->GetMean();
        rms = hU1vsZpt_pfy->GetRMS();
        ...
        delete hU1vsZpt_pfy;
        delete hU1vsZpt_MC_pfy;
        delete hU1vsZpt_MC_raw_pfy;
        delete hU1Diff;
        ...
        mean = hU2vsZpt_pfy->GetMean();
        rms = hU2vsZpt_pfy->GetRMS();
        ...
        delete hU2vsZpt_pfy;
        delete hU2vsZpt_MC_pfy;
        delete hU2vsZpt_MC_raw_pfy;
        delete hU2Diff;
    }
    ...
    char lumitext[100];
    if (lumi < 0.1)
        sprintf(lumitext, "%.1f pb^{-1}  at  #sqrt{s} = 8 TeV", lumi * 1000.);
    else
