    ...
    )
{
    ...
    TFile* inWmunuShapes = new TFile(inputName);
    assert(inWmunuShapes);
    ...
    TH1D* hDataMetp = (TH1D*)datap->createHistogram("pfmet", NBINS);
    hDataMetp->Sumw2();

    RooAbsPdf* wmetp = combine_workspace->pdf("wmp");
    TH1D* hWmunuMetp = (TH1D*)wmetp->createHistogram("pfmet", NBINS);
    hWmunuMetp->Sumw2();

    RooAbsData* wmetpData = combine_workspace->embeddedData("wmunuMETp");
    TH1D* hWmunuMetpWerr = (TH1D*)wmetpData->createHistogram("pfmet", NBINS);
    hWmunuMetpWerr->Sumw2();

    RooAbsPdf* ewkp = combine_workspace->pdf("ewkp");
    TH1D* hEWKMetp = (TH1D*)ewkp->createHistogram("pfmet", NBINS);
    hEWKMetp->Sumw2();

    RooAbsData* datarecoilp;
    RooAbsPdf* wrecoilp;
    RooAbsData* wrecoilpData;
    RooAbsPdf* ewkrecoilp;
    TH1D* hDataRecoilp;
    TH1D* hWmunuRecoilp;
    TH1D* hWmunuRecoilpWerr;
    TH1D* hEWKRecoilp;
    if (noRecoil) {
        datarecoilp = combine_workspace->data("dataMetp");
        hDataRecoilp = (TH1D*)datarecoilp->createHistogram("pfmet", NBINS);
        hDataRecoilp->Sumw2();

        wrecoilp = combine_workspace->pdf("wmp");
        hWmunuRecoilp = (TH1D*)wrecoilp->createHistogram("pfmet", NBINS);
        hWmunuRecoilp->Sumw2();

        wrecoilpData = combine_workspace->embeddedData("wmunuMETp");
        hWmunuRecoilpWerr = (TH1D*)wrecoilpData->createHistogram("pfmet", NBINS);
        hWmunuRecoilpWerr->Sumw2();

        ewkrecoilp = combine_workspace->pdf("ewkp");
        hEWKRecoilp = (TH1D*)ewkrecoilp->createHistogram("pfmet", NBINS);
        hEWKRecoilp->Sumw2();
    } else {
    ...
    for (int i = 0; i < nShapes; ++i) {
        ...
        TFile* inWmunuShapes_alt = new TFile(inputName);
        assert(inWmunuShapes_alt);
        ...
        TH1D* hWmunuMetp_alt = (TH1D*)wmetp_a->createHistogram("pfmet", NBINS);
        hWmunuMetp_alt->Sumw2();

        RooAbsPdf* wrecoilp_a;
        TH1D* hWmunuRecoilp_alt;
        if (noRecoil) {
            wrecoilp_a = combine_wksp_alt->pdf("wmp");
            hWmunuRecoilp_alt = (TH1D*)wrecoilp_a->createHistogram("pfmet", NBINS);
            hWmunuRecoilp_alt->Sumw2();
        } else {
        ...
        for (int i = 0; i <= NBINS; ++i) {
            ...
            if (hWmunuRecoilp->GetBinContent(i) == 0)
                addon = 0;
            ...
            if (hWmunuMetp->GetBinContent(i) == 0)
                addon = 0;
            std::cout << "addon = " << addon << std::endl;
            current += addon * addon;
    ...
    RooHistPdf pdfWmprec("wmprec", "wmprec", *pfmet, wmunuRecoilp, 1);
    RooDataHist ewkRecoilp("ewkRecoilp", "ewkRecoilp", RooArgSet(*pfmet), hEWKRecoilp);
    RooHistPdf pdfEWKprec("ewkprec", "ewkprec", *pfmet, ewkRecoilp, 1);
    ...
    for (int ibin = 1; ibin < hPdfRecoilp->GetNbinsX(); ++ibin) {
        hPdfRecoilp->SetBinError(ibin, hWmunuRecoilp->GetBinError(ibin));
    }
    ...
    char lumitext[100];
    if (lumi < 0.1)
        sprintf(lumitext, "%.1f pb^{-1}  (8 TeV)", lumi * 1000.);
    else
...
{
    if (!hUp || !hDown)
        cout << "TH1TOTGraph: histogram not found !" << endl;
