...
CPepeModel2::CPepeModel2(const char* name, RooRealVar& x, RooRealVar* sigma1, RooRealVar* sigma0)
{
    ...
    a3 = new RooRealVar(a3Name, a3Name, 2.9, 0.3, 6.0);
...
CPepeModel3::CPepeModel3(const char* name, RooRealVar& x, RooRealVar* sigma1, RooRealVar* sigma0)
{
    ...
    char a3Name[50];
    sprintf(a3Name, "a3_%s", name);
    a3 = new RooRealVar(a3Name, a3Name, 10.0, 0.0, 20);
    char a4Name[50];
    sprintf(a4Name, "a4_%s", name);
    a4 = new RooRealVar(a4Name, a4Name, 2.9, 0.3, 6.0);
...
CHistModel::CHistModel(const char* name, RooRealVar& x, TH1D* hist, int intOrder)
{
    ...
    sprintf(vname, "inHist_%s", name);
    inHist = (TH1D*)hist->Clone(vname);
    sprintf(vname, "dataHist_%s", name);
    dataHist = new RooDataHist(vname, vname, RooArgSet(x), inHist);
    sprintf(vname, "histPdf_%s", name);
    model = new RooHistPdf(vname, vname, x, *dataHist, intOrder);
}
