...
{
    if (!(muon->typeBits & baconhep::EMuType::kGlobal) && !(muon->typeBits & baconhep::EMuType::kTracker))
        return kFALSE;
    if (!(muon->typeBits & baconhep::EMuType::kPFMuon))
        return kFALSE;
...
{ // Tight Electron ID from 2017
    ...
    if (fabs(electron->eta) <= ECAL_GAP_LOW) {
        // if(iso >= 0.0354*(tag.Pt()))                                      return kFALSE; // regular ISO
        if (iso / (tag.Pt()) >= 0.0287 + 0.506 / (tag.Pt()))
            return kFALSE; // original
        if (electron->nMissingHits > 1)
            return kFALSE;
        ...
        if (fabs(electron->dPhiIn) >= 0.022)
            return kFALSE; // original
        if (fabs(electron->dEtaIn) >= 0.00255)
            return kFALSE; // original
        if (electron->hovere >= 0.026 + 1.15 / electron->ecalEnergy + 0.0324 * rho / electron->ecalEnergy)
            return kFALSE;
        ...
        // if(iso >= 0.0646*(tag.Pt()))                                      return kFALSE; // regular ISO
        if (iso / (tag.Pt()) >= 0.0445 + 0.963 / (tag.Pt()))
            return kFALSE; // original
        if (electron->nMissingHits > 1)
            return kFALSE;
        ...
        if (fabs(electron->dPhiIn) >= 0.0236)
            return kFALSE; // original
        if (fabs(electron->dEtaIn) >= 0.00501)
            return kFALSE; // original
        // std::cout << "----------------" << std::endl;
        if (electron->hovere >= 0.0188 + 2.06 / electron->ecalEnergy + 0.183 * rho / electron->ecalEnergy)
            return kFALSE;
...
{ // Veto ID  with 2017 loose WP
    ...
    if (fabs(electron->eta) <= ECAL_GAP_LOW) {
        ...
        if (iso / (tag.Pt()) >= 0.198 + 0.506 / (tag.Pt()))
            return kFALSE;
        if (electron->nMissingHits >= 2)
            return kFALSE;
        if (electron->sieie >= 0.0126)
            return kFALSE;
        if (fabs(electron->dPhiIn) >= 0.148)
            return kFALSE;
        if (fabs(electron->dEtaIn) >= 0.00463)
            return kFALSE;
        if (electron->hovere >= 0.05 + 1.16 / (electron->ecalEnergy) + 0.0324 * rho / (electron->ecalEnergy))
            return kFALSE;
        if (fabs(1.0 - electron->eoverp) >= 0.209 * (electron->ecalEnergy))
            return kFALSE;
        if (fabs(electron->d0) >= 0.05)
            return kFALSE;
        if (fabs(electron->dz) >= 0.10)
        ...
        if (iso / (tag.Pt()) >= 0.203 + 0.963 / (tag.Pt()))
            return kFALSE;
        if (electron->nMissingHits >= 3)
            return kFALSE;
        if (electron->sieie >= 0.0457)
            return kFALSE;
        if (fabs(electron->dPhiIn) >= 0.19)
            return kFALSE;
        if (fabs(electron->dEtaIn) >= 0.00814)
            return kFALSE;
        if (electron->hovere >= 0.05 + 2.54 / (electron->ecalEnergy) + 0.183 * rho / (electron->ecalEnergy))
            return kFALSE;
        if (fabs(1.0 - electron->eoverp) >= 0.132 * (electron->ecalEnergy))
            return kFALSE;
        if (fabs(electron->d0) >= 0.10)
            return kFALSE;
        if (fabs(electron->dz) >= 0.20)
...
{ // Medium Electron ID for 2015
    ...
    if ((fabs(electron->eta) > ECAL_GAP_LOW) && (fabs(electron->eta) < ECAL_GAP_HIGH))
        return kFALSE;
    ...
    if (fabs(electron->eta) <= ECAL_GAP_LOW) {
        if (iso >= 0.0766 * (tag.Pt()))
            return kFALSE;
        if (electron->nMissingHits > 2)
            return kFALSE;
        if (electron->sieie >= 0.0101)
            return kFALSE;
        if (fabs(electron->dPhiIn) >= 0.0336)
            return kFALSE;
        if (fabs(electron->dEtaIn) >= 0.0103)
            return kFALSE;
        if (electron->hovere >= 0.0876)
            return kFALSE;
        if (fabs(1.0 - electron->eoverp) >= 0.0174 * (electron->ecalEnergy))
            return kFALSE;
        if (fabs(electron->d0) >= 0.0118)
            return kFALSE;
        if (fabs(electron->dz) >= 0.373)
            return kFALSE;
    } else {
        if (iso >= 0.0678 * (tag.Pt()))
            return kFALSE;
        if (electron->nMissingHits > 1)
            return kFALSE;
        if (electron->sieie >= 0.0283)
            return kFALSE;
        if (fabs(electron->dPhiIn) >= 0.114)
            return kFALSE;
        if (fabs(electron->dEtaIn) >= 0.00733)
            return kFALSE;
        if (electron->hovere >= 0.0678)
            return kFALSE;
        if (fabs(1.0 - electron->eoverp) >= 0.0898 * (electron->ecalEnergy))
            return kFALSE;
        if (fabs(electron->d0) >= 0.0739)
            return kFALSE;
        if (fabs(electron->dz) >= 0.602)
            return kFALSE;
    }
...
{ // Tight Electron ID from 2015
    ...
    if (fabs(electron->eta) <= ECAL_GAP_LOW) {
        if (iso >= 0.0354 * (tag.Pt()))
            return kFALSE; // regular ISO
        if (electron->nMissingHits > 2)
            return kFALSE;
        if (electron->sieie >= 0.0101)
            return kFALSE;
        if (fabs(electron->dPhiIn) >= 0.0336)
            return kFALSE;
        if (fabs(electron->dEtaIn) >= 0.00926)
            return kFALSE;
        if (electron->hovere >= 0.0597)
            return kFALSE;
        if (fabs(1.0 - electron->eoverp) >= 0.012 * (electron->ecalEnergy))
            return kFALSE;
        if (fabs(electron->d0) >= 0.0111)
            return kFALSE;
        if (fabs(electron->dz) >= 0.0466)
            return kFALSE;
    } else {
        if (iso >= 0.0646 * (tag.Pt()))
            return kFALSE; // regular ISO
        if (electron->nMissingHits > 1)
            return kFALSE;
        if (electron->sieie >= 0.0279)
            return kFALSE;
        if (fabs(electron->dPhiIn) >= 0.0918)
            return kFALSE;
        if (fabs(electron->dEtaIn) >= 0.00724)
            return kFALSE;
        if (electron->hovere >= 0.0615)
            return kFALSE;
        if (fabs(1.0 - electron->eoverp) >= 0.00999 * (electron->ecalEnergy))
            return kFALSE;
        if (fabs(electron->d0) >= 0.0351)
            return kFALSE;
        if (fabs(electron->dz) >= 0.417)
            return kFALSE;
    }
...
{ // Tight Electron ID from 2015
    ...
    if (fabs(electron->eta) <= ECAL_GAP_LOW) {
        ...
        if (fabs(1.0 - electron->eoverp) >= 0.012 * (electron->ecalEnergy))
            return kFALSE;
        if (fabs(electron->d0) >= 0.0111)
            std::cout << "FAIL d0 " << std::endl; //return kFALSE;
        if (fabs(electron->dz) >= 0.0466)
            std::cout << "FAIL dz " << std::endl; //return kFALSE;
    } else {
        if (iso >= 0.0646 * (tag.Pt()))
            return kFALSE; // original
...
{ // Veto ID  for 2015 Loose WP
    ...
    if (fabs(electron->eta) <= ECAL_GAP_LOW) {
        ...
        if (electron->nMissingHits > 2)
            return kFALSE;
        if (electron->sieie > 0.0114)
            return kFALSE;
        if (fabs(electron->dPhiIn) > 0.216)
            return kFALSE;
        if (fabs(electron->dEtaIn) > 0.0152)
            return kFALSE;
        if (electron->hovere > 0.181)
            return kFALSE;
        if (fabs(1.0 - electron->eoverp) > 0.207 * (electron->ecalEnergy))
            return kFALSE;
        if (fabs(electron->d0) > 0.0564)
            return kFALSE;
        if (fabs(electron->dz) > 0.472)
            return kFALSE;
    } else {
        // endcap
        if (iso > 0.144 * (tag.Pt()))
        ...
        if (electron->nMissingHits > 3)
            return kFALSE;
        if (electron->sieie > 0.0352)
            return kFALSE;
        if (fabs(electron->dPhiIn) > 0.237)
            return kFALSE;
        if (fabs(electron->dEtaIn) > 0.0113)
            return kFALSE;
        if (electron->hovere > 0.116)
            return kFALSE;
        if (fabs(1.0 - electron->eoverp) > 0.174 * (electron->ecalEnergy))
            return kFALSE;
        if (fabs(electron->d0) > 0.222)
            return kFALSE;
        if (fabs(electron->dz) > 0.921)
            return kFALSE;
    }
...
Bool_t isMuonTrigger(baconhep::TTrigger triggerMenu, TriggerBits hltBits, Bool_t isData, Bool_t is13TeV = true)
{
    if (isData)
        ...
        if (is13TeV)
            return triggerMenu.pass("HLT_Mu17_v*", hltBits); // 13 TeV
        else
            return triggerMenu.pass("HLT_HIMu17_v*", hltBits); // 5 TeV
    }
}
...
{
    if (isData)
        return triggerMenu.passObj("HLT_HIMu17_v*", "hltL3fL1sMu10lqL1f0L2f10L3Filtered17", hltMatchBits); // both 5 & 13
    else {
        if (is13TeV)
            return triggerMenu.passObj("HLT_Mu17_v*", "hltL3fL1sMu10lqL1f0L2f10L3Filtered17", hltMatchBits); // 13 TeV
        else
            return triggerMenu.passObj("HLT_HIMu17_v*", "hltL3fL1sMu10lqL1f0L2f10L3Filtered17", hltMatchBits); // 5 TeV
    }
}
...
{
    if (isL1)
        return triggerMenu.passObj("HLT_HIMu17_v*", "hltL3fL1sMu10lqL1f0L2f10L3Filtered17", hltMatchBits);
    else
        return triggerMenu.passObj("HLT_Mu17_v*", "hltL3fL1sMu10lqL1f0L2f10L3Filtered17", hltMatchBits);
}
...
Bool_t isEleTrigger(baconhep::TTrigger triggerMenu, TriggerBits hltBits, Bool_t isData, Bool_t is13TeV = true)
{
    if (isData) {
        if (is13TeV)
            return triggerMenu.pass("HLT_Ele20_WPLoose_Gsf_v*", hltBits); // 13 TeV
        else
        ...
        if (is13TeV)
            ...
            return triggerMenu.pass("HLT_HIEle17_WPLoose_Gsf_v*", hltBits); // 5 TeV
    }
}
Bool_t isEleTriggerObj(baconhep::TTrigger triggerMenu, TriggerObjects hltMatchBits, Bool_t isL1, Bool_t isData, Bool_t is13TeV = true)
{
    if (isData) {
        if (is13TeV)
            return triggerMenu.passObj("HLT_Ele20_WPLoose_Gsf_v*", "hltEle20WPLoose1GsfTrackIsoFilter", hltMatchBits);
        else
...
{ //2017
    if (fabs(eta) < 1.0)
        ...
        return -1; // This should never happen, a cut on |eta|<2.5 is applied before this function is used.
}
...
Double_t getEffAreaMu(const Double_t eta)
{
    ...
    if (fabs(eta) < 0.8)
        ...
        return -1; // This should never happen, a cut on |eta|<2.4 is applied in selection.
}
