...
CDoubleExp::CDoubleExp(const char* name, RooRealVar& m)
{
    ...
    sprintf(vname, "t1_%s", name);
    t1 = new RooRealVar(vname, vname, -0.20, -1., 0.);
    sprintf(vname, "t2_%s", name);
    t2 = new RooRealVar(vname, vname, -0.05, -1., 0.);
    sprintf(vname, "frac_%s", name);
    frac = new RooRealVar(vname, vname, 0.50, 0., 1.);
