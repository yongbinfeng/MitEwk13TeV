...
void CPlot::AddGraph(TGraph* gr, TString label, TString drawopt, int color, int marksty, int linesty)
{
    ...
    if (!fLeg)
        ...
        fLeg->SetY1(fLeg->GetY1() - 0.06);
    if ((drawopt.Contains("L", TString::kIgnoreCase) || drawopt.Contains("C", TString::kIgnoreCase))
    ...
    int linecolor, int linesty, int fillcolor, TString label)
{
    ...
    if (fillcolor < 0)
        fLeg->AddEntry(box, label, "L");
    else
        fLeg->AddEntry(box, label, "F");
}
...
void CPlot::Draw(TCanvas* c, bool doSave, TString format, Int_t subpad)
{
    ...
    if (nHist1D > 0) {
        ...
        for (uint i = 0; i < fItems.size(); i++) {
            if (fItems[i].hist1D == 0)
                continue;
            if (fStack && fStack->GetHists()->Contains(fItems[i].hist1D))
                continue;
    ...
    if (fShowStats) {
        ...
        for (uint i = 0; i < vHists.size(); i++) {
            ...
            if (x / 100) {
                stat[3 * i].SetNDC();
                stat[3 * i].SetTextAlign(13);
                stat[3 * i].SetTextSize(0.03);
                ...
                sval[3 * i].SetNDC();
                sval[3 * i].SetTextAlign(33);
                sval[3 * i].SetTextSize(0.03);
            ...
            if (x / 10) {
                stat[3 * i + 1].SetNDC();
                stat[3 * i + 1].SetTextAlign(13);
                stat[3 * i + 1].SetTextSize(0.03);
                ...
                sval[3 * i + 1].SetNDC();
                sval[3 * i + 1].SetTextAlign(33);
                sval[3 * i + 1].SetTextSize(0.03);
            ...
            if (x) {
                stat[3 * i + 2].SetNDC();
                stat[3 * i + 2].SetTextAlign(13);
                stat[3 * i + 2].SetTextSize(0.03);
                ...
                sval[3 * i + 2].SetNDC();
                sval[3 * i + 2].SetTextAlign(33);
                sval[3 * i + 2].SetTextSize(0.03);
