    ...
    if (mceff_vs_eta_per_ptv.size() > 0 && eff_vs_eta_per_ptv.size() > 0) {
        for (UInt_t ig = 0; ig < mceff_vs_eta_per_ptv.size(); ig++) {
            ...
            for (int l = 0; l < grMCEffEta->GetN(); l++) {
                ...
                if (d + grMCEffEta->GetErrorYhigh(l) > 1)
                    grMCEffEta->SetPointEYhigh(l, 0);
                ...
                // Don't draw error bars above 1
                if (d + grDataEffEta->GetErrorYhigh(l) > 1)
                    grDataEffEta->SetPointEYhigh(l, 0);
            }
            ...
            // Remove ECAL gap
            if (xaxislabel.CompareTo("Supercluster") == 0) {
                grMCEffEta->RemovePoint(2);
                grMCEffEta->RemovePoint(8);
                grDataEffEta->RemovePoint(2);
                grDataEffEta->RemovePoint(8);
                grScaleEta->RemovePoint(2);
                grScaleEta->RemovePoint(8);
        ...
        int binEdgeLimit = 0;
        if ((etaBinEdgesv.size() - 1) % 2 == 0)
            binEdgeLimit = (etaBinEdgesv.size() - 1) / 2;
