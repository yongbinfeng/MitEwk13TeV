    ...
    )
{
    ...
    while (getline(ifs, line)) {
        ...
        if (state == 0) {
            ...
            ss >> edge;
            if (state == 1) {
                ptBinEdgesv.push_back(edge);
            } else if (state == 2) {
                etaBinEdgesv.push_back(edge);
            } else if (state == 3) {
                phiBinEdgesv.push_back(edge);
            } else if (state == 4) {
                npvBinEdgesv.push_back(edge);
            }
        }
    }
    ifs.close();
    ...
    if (sigModPass == 0 && sigModFail == 0) { // probe counting
        ...
        if (opts[1]) {
            ...
            if (doAbsEta)
                plotEffEta.SetXTitle("probe |#eta|");
            ...
            if (doAbsEta)
                plotEffEta2.SetXTitle("probe |#eta|");
        ...
        if (opts[5]) {
            ...
            if (doAbsEta)
                plotEffEtaPhi.SetXTitle("probe |#eta|");
    ...
    if (hEffEtaPt->GetEntries() > 0) {
        ...
        effetapt.printEff(txtfile);
        txtfile << endl;
        effetapt.printErrLow(txtfile);
        txtfile << endl;
        effetapt.printErrHigh(txtfile);
        txtfile << endl;
        txtfile << endl;

        effetapt.printEffLatex(latexfile);
        latexfile << endl;
    }

    if (hEffEtaPhi->GetEntries() > 0) {
        effetaphi.loadEff(hEffEtaPhi, hErrlEtaPhi, hErrhEtaPhi);
        effetaphi.printEff(txtfile);
        txtfile << endl;
        effetaphi.printErrLow(txtfile);
        txtfile << endl;
        effetaphi.printErrHigh(txtfile);
        txtfile << endl;
        txtfile << endl;
    }
    ...
    const Double_t zptLo, const Double_t zptHi)
{
    ...
    if (weightPowPyth > 10 || weightPowPyth < 0)
        weightPowPyth = 1;
