    ...
    )
{
    ...
    if (mceff_vs_pt_per_etav.size() > 0 && eff_vs_pt_per_etav.size() > 0) {
        for (UInt_t ig = 0; ig < mceff_vs_pt_per_etav.size(); ig++) {
            ...
            if (yaxislabel.CompareTo("Supercluster") == 0) {
                ptlow = 25;
                pthigh = 100;
            } else if (yaxislabel.CompareTo("Muon") == 0) {
                ptlow = 25;
                pthigh = 100;
            }
            ...
            for (int l = 0; l < grMCEffPt->GetN(); l++) {
                ...
                if (d + grMCEffPt->GetErrorYhigh(l) > 1)
                    grMCEffPt->SetPointEYhigh(l, 0);
                ...
                // Don't draw error bars above 1
                if (d + grDataEffPt->GetErrorYhigh(l) > 1)
                    grDataEffPt->SetPointEYhigh(l, 0);
            }
