...
CBreitWignerConvCrystalBall::CBreitWignerConvCrystalBall(RooRealVar& m, const Bool_t pass)
{
    ...
    if (pass) {
        sprintf(vname, "mean%s", name);
        mean = new RooRealVar(vname, vname, 0, -10, 10);
        sprintf(vname, "sigma%s", name);
        sigma = new RooRealVar(vname, vname, 1, 0.1, 5);
        sprintf(vname, "alpha%s", name);
        alpha = new RooRealVar(vname, vname, 5, 0, 20);
        ...
        sprintf(vname, "sigma%s", name);
        sigma = new RooRealVar(vname, vname, 1, 0.1, 6);
        sprintf(vname, "alpha%s", name);
        alpha = new RooRealVar(vname, vname, 5, 0, 20);
        sprintf(vname, "n%s", name);
        n = new RooRealVar(vname, vname, 1, 0, 10);
...
CMCTemplateConvGaussian::CMCTemplateConvGaussian(RooRealVar& m, TH1D* hist, const Bool_t pass, const int ibin, RooRealVar* sigma0, int intOrder)
{
    char name[10];
    if (pass)
        sprintf(name, "%s_%i", "Pass", ibin);
    else
        sprintf(name, "%s_%i", "Fail", ibin);
    /*
  if(xbinLo==0) {
...
*/
    ...
    if (pass) {
        sprintf(vname, "mean%s", name);
        mean = new RooRealVar(vname, vname, 0.3, -2, 2);
        ...
        sprintf(vname, "mean%s", name);
        mean = new RooRealVar(vname, vname, 1.0, -2, 2);
        //mean->setConstant(kTRUE);
        if (sigma0) {
            sigma = sigma0;
        } else {
    ...
    sprintf(vname, "dataHist%s", name);
    dataHist = new RooDataHist(vname, vname, RooArgSet(m), inHist);
    sprintf(vname, "histPdf%s", name);
    histPdf = new RooHistPdf(vname, vname, m, *dataHist, intOrder);
    sprintf(vname, "signal%s", name);
...
CMCDatasetConvGaussian::CMCDatasetConvGaussian(RooRealVar& m, TTree* tree, const Bool_t pass, RooRealVar* sigma0)
{
    ...
    if (pass) {
        ...
        if (sigma0) {
            ...
            sprintf(vname, "sigma%s", name);
            sigma = new RooRealVar(vname, vname, 2, 0, 5);
        }
        sprintf(vname, "gaus%s", name);
        gaus = new RooGaussian(vname, vname, m, *mean, *sigma);
    } else {
    ...
    sprintf(vname, "dataSet%s", name);
    dataSet = new RooDataSet(vname, vname, inTree, RooArgSet(m));
    sprintf(vname, "keysPdf%s", name);
    keysPdf = new RooKeysPdf(vname, vname, m, *dataSet, RooKeysPdf::NoMirror, 1);
    sprintf(vname, "signal%s", name);
    model = new RooFFTConvPdf(vname, vname, m, *keysPdf, *gaus);
}
