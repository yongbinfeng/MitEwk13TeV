    ...
    )
{
    ...
    for (int iEtaBin = 0; iEtaBin < muEtaNB; iEtaBin++) {
        for (int iPtBin = 0; iPtBin < muPtNB; iPtBin++) {
            ...
            double norm = h_mll_amcnlo[iEtaBin][iPtBin]->Integral();
            h_mll_amcnlo[iEtaBin][iPtBin]->Scale(1 / norm);
            norm = 0;
            h_mll_photos[iEtaBin][iPtBin] = (TH1D*)inPhotos->Get(Form("h_mll_etaBin%d_ptBin%d", iEtaBin, iPtBin));
            norm = h_mll_photos[iEtaBin][iPtBin]->Integral();
            h_mll_photos[iEtaBin][iPtBin]->Scale(1 / norm);
            norm = 0;
            h_mll_pythia[iEtaBin][iPtBin] = (TH1D*)inPythia->Get(Form("h_mll_etaBin%d_ptBin%d", iEtaBin, iPtBin));
            norm = h_mll_pythia[iEtaBin][iPtBin]->Integral();
            h_mll_pythia[iEtaBin][iPtBin]->Scale(1 / norm);
            norm = 0;
        }
    }
    ...
    if (effType == eSCEff)
        intree->SetBranchAddress("matchGenSCEff", &matchGenSCEff);
  ...
  */
    ...
    for (UInt_t ientry = 0; ientry < intree->GetEntries(); ientry++) {
        ...
        if (effType == eSCEff) {
            if (doGenMatch && !matchGenSCEff)
                continue;
        } else {
            if (doGenMatch && !matchGen)
                continue;
        }
        ...
        if (effType == eHLTEff) {
            ...
            if (category == eEleEle2HLT) {
                pass = kTRUE;
            } else if (category == eEleEle1HLT1L1) {
                pass = kFALSE;
            } else if (category == eEleEle1HLT) {
                pass = kFALSE;
            } else if (category == eEleEleNoSel) {
                continue;
            } else {
                continue;
            }
        ...
        if (doWeighted) {
            ...
            for (int iEtaBin = 0; iEtaBin < muEtaNB; iEtaBin++) {
                if (geneta < muEtaRange[iEtaBin] || geneta > muEtaRange[iEtaBin + 1])
                    continue;
                // std::cout << "eta bin = " << iEtaBin << std::endl;
                for (int iPtBin = 0; iPtBin < muPtNB; iPtBin++) {
                    if (genpt < muPtRange[iPtBin] || genpt > muPtRange[iPtBin + 1])
                        continue;
