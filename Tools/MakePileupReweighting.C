    ...
    TString outfile = "pileup_weights_2017H_v2.root")
{
    ...
    TH1D* h_data = new TH1D("npv_rw", "npv_rw", 40, 0, 40);
    h_data->Sumw2();
    ...
    t_data->SetBranchAddress("Info", &info);
    TBranch* infoBr = t_data->GetBranch("Info");
    t_data->SetBranchAddress("PV", &vertexArr);
    TBranch* vertexBr = t_data->GetBranch("PV");
    ...
    for (Int_t i = 0; i < t_data->GetEntries(); i++) {
        if (i % 100000 == 0)
            std::cout << "On Entry " << i << " out of " << t_data->GetEntries() << "..." << 100 * i / t_data->GetEntries() << std::endl;
        vertexArr->Clear();
        vertexBr->GetEntry(i);
        infoBr->GetEntry(i);
    ...
    TH1D* h_mc = new TH1D("npv_mc", "npv_mc", 40, 0, 40);
    h_mc->Sumw2();

    t_mc->SetBranchAddress("Info", &info);
    infoBr = t_mc->GetBranch("Info");
    t_mc->SetBranchAddress("GenEvtInfo", &gen);
    TBranch* genBr = t_mc->GetBranch("GenEvtInfo");
    t_mc->SetBranchAddress("PV", &vertexArr);
    vertexBr = t_mc->GetBranch("PV");

    Double_t nZeroMC = 0, nGoodMC = 0;
    std::cout << "MC File " << std::endl;
    for (Int_t i = 0; i < t_mc->GetEntries(); i++) {
        if (i % 100000 == 0)
            std::cout << "On Entry " << i << " out of " << t_mc->GetEntries() << "..." << 100 * i / t_mc->GetEntries() << std::endl;
        infoBr->GetEntry(i);
        genBr->GetEntry(i);
        vertexArr->Clear();
        vertexBr->GetEntry(i);
    ...
    h_mc->SetFillColor(798);
    h_mc->SetLineColor(797);
    h_mc->SetLineWidth(3);
