...
void xsecSummary_muon5()
{
    ...
    // input parameter
    double blind = 1;
    Double_t xsec_wp = 4387;
    blind = theo_wp / xsec_wp;
    xsec_wp = xsec_wp * blind;
    const Double_t xsec_wp_stat = 0;
    const Double_t xsec_wp_sys = sqrt(pow(25 * blind, 2) + th_wp * th_wp);
    const Double_t xsec_wp_lumi = 209 * blind;
    Double_t xsec_wm = 2885;
    blind = theo_wm / xsec_wm;
    xsec_wm = xsec_wm * blind;
    const Double_t xsec_wm_stat = 0;
    const Double_t xsec_wm_sys = sqrt(pow(23 * blind, 2) + th_wm * th_wm);
    const Double_t xsec_wm_lumi = 138 * blind;
    Double_t xsec_w = 7265;
    blind = theo_w / xsec_w;
    xsec_w = xsec_w * blind;
    const Double_t xsec_w_stat = 0;
    const Double_t xsec_w_sys = sqrt(pow(35 * blind, 2) + th_w * th_w);
    const Double_t xsec_w_lumi = 347 * blind;
    Double_t xsec_z = 667;
    blind = theo_z / xsec_z;
    xsec_z = xsec_z * blind;
    const Double_t xsec_z_stat = 0;
    const Double_t xsec_z_sys = sqrt(pow(4 * blind, 2) + th_z * th_z);
    const Double_t xsec_z_lumi = 32 * blind;
    Double_t xsec_wr = 1.51834;
    blind = theo_wr / xsec_wr;
    xsec_wr = xsec_wr * blind;
    const Double_t xsec_wr_stat = 0.0;
    const Double_t xsec_wr_sys = sqrt(pow(0.00609 * blind, 2) + th_wr * th_wr);
    Double_t xsec_wpr = 6.5774;
    blind = theo_wpr / xsec_wpr;
    xsec_wpr = xsec_wpr * blind;
    const Double_t xsec_wpr_stat = 0.0;
    const Double_t xsec_wpr_sys = sqrt(pow(0.0496876 * blind, 2) + th_wpr * th_wpr);
    Double_t xsec_wmr = 4.33528;
    blind = theo_wmr / xsec_wmr;
    xsec_wmr = xsec_wmr * blind;
    const Double_t xsec_wmr_stat = 0.0;
    const Double_t xsec_wmr_sys = sqrt(pow(0.0326268 * blind, 2) + th_wmr * th_wmr);
    Double_t xsec_wz = 10.917;
    blind = theo_wz / xsec_wz;
    xsec_wz = xsec_wz * blind;
    const Double_t xsec_wz_stat = 0.0;
    const Double_t xsec_wz_sys = sqrt(pow(0.0720251 * blind, 2) + th_wz * th_wz);
