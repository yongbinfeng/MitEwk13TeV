...
void make_fiducial(const TString infilename = "make_fiducial.txt")
{
    ...
    wpe_stat *= acc_wpe;
    wpe_lumi *= acc_wpe;
    Double_t wpe_new = wpe_sys * wpe / 100;
    wme *= acc_wme;
    wme_stat *= acc_wme;
    wme_lumi *= acc_wme;
    Double_t wme_new = wme_sys * wme / 100;

    we *= acc_we;
    we_stat *= acc_we;
    we_lumi *= acc_we;
    Double_t we_new = we_sys * we / 100;
    wpm *= acc_wpm;
    wpm_stat *= acc_wpm;
    wpm_lumi *= acc_wpm;
    Double_t wpm_new = wpm_sys * wpm / 100;

    wmm *= acc_wmm;
    wmm_stat *= acc_wmm;
    wmm_lumi *= acc_wmm;
    Double_t wmm_new = wmm_sys * wmm / 100;
    wm *= acc_wm;
    wm_stat *= acc_wm;
    wm_lumi *= acc_wm;
    Double_t wm_new = wm_sys * wm / 100;

    ze *= acc_ze;
    ze_stat *= acc_ze;
    ze_lumi *= acc_ze;
    Double_t ze_new = ze_sys * ze / 100;
    zm *= acc_zm;
    zm_stat *= acc_zm;
    zm_lumi *= acc_zm;
    Double_t zm_new = zm_sys * zm / 100;

    wre *= acc_wpe / acc_wme;
    wre_stat *= acc_wpe / acc_wme;
    wre_lumi *= acc_wpe / acc_wme;
    wre_sys *= wre / 100;
    wre_lumi = 0;

    wrm *= acc_wpm / acc_wmm;
    wrm_stat *= acc_wpm / acc_wmm;
    wrm_lumi *= acc_wpm / acc_wmm;
    wrm_sys *= wrm / 100;
    wrm_lumi = 0;

    wzre *= acc_we / acc_ze;
    wzre_stat *= acc_we / acc_ze;
    wzre_lumi *= acc_we / acc_ze;
    wzre_sys *= wzre / 100;
    wzre_lumi = 0;

    wzrm *= acc_wm / acc_zm;
    wzrm_stat *= acc_wm / acc_zm;
    wzrm_lumi *= acc_wm / acc_zm;
    wzrm_sys *= wzrm / 100;
    wzrm_lumi = 0;

    wpzre *= acc_wpe / acc_ze;
    wpzre_stat *= acc_wpe / acc_ze;
    wpzre_lumi *= acc_wpe / acc_ze;
    wpzre_sys *= wpzre / 100;
    wpzre_lumi = 0;

    wpzrm *= acc_wpm / acc_zm;
    wpzrm_stat *= acc_wpm / acc_zm;
    wpzrm_lumi *= acc_wpm / acc_zm;
    wpzrm_sys *= wpzrm / 100;
    wpzrm_lumi = 0;

    wmzre *= acc_wme / acc_ze;
    wmzre_stat *= acc_wme / acc_ze;
    wmzre_lumi *= acc_wme / acc_ze;
    wmzre_sys *= wmzre / 100;
    wmzre_lumi = 0;

    wmzrm *= acc_wmm / acc_zm;
    wmzrm_stat *= acc_wmm / acc_zm;
    wmzrm_lumi *= acc_wmm / acc_zm;
    wmzrm_sys *= wmzrm / 100;
    wmzrm_lumi = 0;
