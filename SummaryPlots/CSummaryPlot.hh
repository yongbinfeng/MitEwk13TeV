...
void CSummaryPlot::Draw(TCanvas* c, TString format)
{
    ...
    if (lumiErr[0] > 0)
        sprintf(buffer, "%s #pm %.2f_{lumi} nb", buffer, lumiErr[0]);
    ...
    if (lumiErr[1] > 0)
        sprintf(buffer, "%s #pm %.2f_{lumi} nb", buffer, lumiErr[1]);
    ...
    if (lumiErrComb > 0)
        sprintf(buffer, "%s #pm %.2f_{lumi} nb", buffer, lumiErrComb);
