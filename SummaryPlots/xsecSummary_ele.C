...
void xsecSummary_ele()
{
    ...
    const Double_t theo_wp = 11603;
    const Double_t theo_wp_unc = 199;
    const Double_t theo_wm = 8549;
    const Double_t theo_wm_unc = 144;
    const Double_t theo_w = 20152;
    const Double_t theo_w_unc = 341;
    const Double_t theo_z = 1933;
    const Double_t theo_z_unc = 35;
    const Double_t theo_wr = 1.357;
    const Double_t theo_wr_unc = 0.007;
    const Double_t theo_wpr = 6.00;
    const Double_t theo_wpr_unc = 0.04;
    const Double_t theo_wmr = 4.42;
    const Double_t theo_wmr_unc = 0.03;
    const Double_t theo_wz = 10.43;
    const Double_t theo_wz_unc = 0.07;
    ...
    // input parameter
    double blind = 1;
    Double_t xsec_wp = 12167;
    blind = theo_wp / xsec_wp;
    xsec_wp = xsec_wp * blind;
    const Double_t xsec_wp_stat = 0;
    const Double_t xsec_wp_sys = sqrt(pow(91 * blind, 2) + th_wp * th_wp);
    const Double_t xsec_wp_lumi = 207 * blind;
    Double_t xsec_wm = 9006;
    blind = theo_wm / xsec_wm;
    xsec_wm = xsec_wm * blind;
    const Double_t xsec_wm_stat = 0;
    const Double_t xsec_wm_sys = sqrt(pow(58 * blind, 2) + th_wm * th_wm);
    const Double_t xsec_wm_lumi = 153 * blind;
    Double_t xsec_w = 21166;
    blind = theo_w / xsec_w;
    xsec_w = xsec_w * blind;
    const Double_t xsec_w_stat = 0;
    const Double_t xsec_w_sys = sqrt(pow(133 * blind, 2) + th_w * th_w);
    const Double_t xsec_w_lumi = 360 * blind;
    Double_t xsec_z = 1986;
    blind = theo_z / xsec_z;
    xsec_z = xsec_z * blind;
    const Double_t xsec_z_stat = 0;
    const Double_t xsec_z_sys = sqrt(pow(15 * blind, 2) + th_z * th_z);
    const Double_t xsec_z_lumi = 34 * blind;
    Double_t xsec_wr = 1.35123;
    blind = theo_wr / xsec_wr;
    xsec_wr = xsec_wr * blind;
    const Double_t xsec_wr_stat = 0.0;
    const Double_t xsec_wr_sys = sqrt(pow(0.0101492 * blind, 2) + th_wr * th_wr);
    Double_t xsec_wpr = 6.12483;
    blind = theo_wpr / xsec_wpr;
    xsec_wpr = xsec_wpr * blind;
    const Double_t xsec_wpr_stat = 0.0;
    const Double_t xsec_wpr_sys = sqrt(pow(0.05215 * blind, 2) + th_wmr * th_wmr);
    Double_t xsec_wmr = 4.53371;
    blind = theo_wmr / xsec_wmr;
    xsec_wmr = xsec_wmr * blind;
    const Double_t xsec_wmr_stat = 0.0;
    const Double_t xsec_wmr_sys = sqrt(pow(0.0387496 * blind, 2) + th_wpr * th_wpr);
    Double_t xsec_wz = 10.6556;
    blind = theo_wz / xsec_wz;
    xsec_wz = xsec_wz * blind;
    const Double_t xsec_wz_stat = 0.0;
    const Double_t xsec_wz_sys = sqrt(pow(0.080893 * blind, 2) + th_wz * th_wz);
    ;

    const Double_t ratio_wp = xsec_wp / theo_wp;
    const Double_t ratio_wp_the = theo_wp_unc * xsec_wp / (theo_wp * theo_wp);
    const Double_t ratio_wp_exp = sqrt(xsec_wp_stat * xsec_wp_stat + xsec_wp_sys * xsec_wp_sys) / (theo_wp);

    const Double_t ratio_wm = xsec_wm / theo_wm;
    const Double_t ratio_wm_the = theo_wm_unc * xsec_wm / (theo_wm * theo_wm);
    const Double_t ratio_wm_exp = sqrt(xsec_wm_stat * xsec_wm_stat + xsec_wm_sys * xsec_wm_sys) / (theo_wm);

    const Double_t ratio_w = xsec_w / theo_w;
    const Double_t ratio_w_the = theo_w_unc * xsec_w / (theo_w * theo_w);
    const Double_t ratio_w_exp = sqrt(xsec_w_stat * xsec_w_stat + xsec_w_sys * xsec_w_sys) / (theo_w);

    const Double_t ratio_z = xsec_z / theo_z;
    const Double_t ratio_z_the = theo_z_unc * xsec_z / (theo_z * theo_z);
    const Double_t ratio_z_exp = sqrt(xsec_z_stat * xsec_z_stat + xsec_z_sys * xsec_z_sys) / (theo_z);

    const Double_t ratio_wr = xsec_wr / theo_wr;
    const Double_t ratio_wr_the = theo_wr_unc * xsec_wr / (theo_wr * theo_wr);
    const Double_t ratio_wr_exp = sqrt(xsec_wr_stat * xsec_wr_stat + xsec_wr_sys * xsec_wr_sys) / (theo_wr);

    const Double_t ratio_wpr = xsec_wpr / theo_wpr;
    const Double_t ratio_wpr_the = theo_wpr_unc * xsec_wpr / (theo_wpr * theo_wpr);
    const Double_t ratio_wpr_exp = sqrt(xsec_wpr_stat * xsec_wpr_stat + xsec_wpr_sys * xsec_wpr_sys) / (theo_wpr);

    const Double_t ratio_wmr = xsec_wmr / theo_wmr;
    const Double_t ratio_wmr_the = theo_wmr_unc * xsec_wmr / (theo_wmr * theo_wmr);
    const Double_t ratio_wmr_exp = sqrt(xsec_wmr_stat * xsec_wmr_stat + xsec_wmr_sys * xsec_wmr_sys) / (theo_wmr);

    const Double_t ratio_wz = xsec_wz / theo_wz;
    const Double_t ratio_wz_the = theo_wz_unc * xsec_wz / (theo_wz * theo_wz);
