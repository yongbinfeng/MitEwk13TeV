...
void plotMichael();
void plotGautier();
void Drawband(float _xmin, float _xmax, float _ymin, float _ymax,
...
void Colliders()
{
    ...
    float energy = 0.5;
    txt_->SetText(energy, ytxt_, Form("%-2.1f", energy));
    txt_->DrawClone();
    energy = 1;
    txt_->SetText(energy, ytxt_, Form("%-1.0f", energy));
    txt_->DrawClone();
    energy = 2;
    txt_->SetText(energy, ytxt_, Form("%-1.0f", energy));
    txt_->DrawClone();
    ...
    txt_->DrawClone();
    energy = 20;
    txt_->SetText(energy, ytxt_, Form("%-2.0f", energy));
    txt_->DrawClone();
    energy = 22;
    ...
    txt_->SetText(energy, 31000, "W");
    txt_->DrawClone();
    txt_->SetText(energy, 20000, "W^{+}");
    txt_->DrawClone();
    txt_->SetText(energy, 13000, "W^{-}");
    txt_->DrawClone();
    txt_->SetText(energy, 3000, "Z");
    txt_->DrawClone();
...
void plotMichael()
{
    ...
    if (plotATLAS) {
        rts[0] += 0.2;
    } // offset needed for seeing points
    //
    ex[0] = 0.001;
    csW[0] = 10300.;
    stat = 20.;
...
void abLabel(const char* label, float posx, float posy, float size)
{
    TLatex latex;
    latex.SetNDC();
