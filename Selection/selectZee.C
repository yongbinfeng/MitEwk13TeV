        ...
        for (UInt_t ifile = 0; ifile < nfiles; ifile++) {
            ...
            for (UInt_t ientry = IBEGIN; ientry < IEND; ientry++) {
                ...
                for (Int_t i1 = 0; i1 < electronArr->GetEntriesFast(); i1++) {
                    ...
                    if (!isEleTriggerObj(triggerMenu, tag->hltMatchBits, kFALSE, isData, is13TeV))
                        continue;
                    if (El_Pt < tagPt)
                        continue;
                ...
                for (Int_t j = 0; j < scArr->GetEntriesFast(); j++) {
                    ...
                    if (El_Pt < PT_CUT)
                        continue;
                    if (passID && eleProbe && passEleMediumID(eleProbe, vEleProbe, info->rhoIso) && El_Pt < probePt)
                        continue;
                    if (passID && eleProbe && !passEleMediumID(eleProbe, vEleProbe, info->rhoIso))
                        continue;
                    if (passID && !eleProbe)
                        continue;
                    if (!passID && eleProbe && !passEleMediumID(eleProbe, vEleProbe, info->rhoIso) && El_Pt < probePt)
                        continue;
                    if (!passID && !eleProbe && El_Pt < probePt)
                        continue;
                    if (!passID && eleProbe && passEleMediumID(eleProbe, vEleProbe, info->rhoIso))
                        passID = true;
                    ...
                    if (eleProbe)
                        vProbe_raw.SetPtEtaPhiM(eleProbe->pt, eleProbe->eta, eleProbe->phi, ELE_MASS);
                    vProbeSC = (eleProbe) ? vEleProbeSC : vProbe;
                    // lep2EcalE = probeEcalEnergy_tmp;

                    trkIso2 = (eleProbe) ? eleProbe->trkIso : -1;
                    pfCombIso2 = (eleProbe) ? eleProbe->chHadIso + TMath::Max(eleProbe->neuHadIso + eleProbe->gammaIso - (info->rhoIso) * getEffAreaEl(vEleProbe.Eta()), 0.) : -1;
