               ...
               const Int_t ISample = -1)
{
    ...
    if (h_rw == NULL || h_rw_up == NULL || h_rw_down == NULL)
        cout << "WARNING h_rw == NULL" << endl;
    ...
    for (UInt_t isam = 0; isam < samplev.size(); isam++)
    {
        ...
        for (UInt_t ifile = 0; ifile < nfiles; ifile++)
        {
            ...
            for (UInt_t ientry = IBEGIN; ientry < IEND; ientry++)
            {
                ...
                { // i guess should fix this if it needs PU weighting
                    hGenWeights->Fill(0.0, 1.0);
                }

                // cout << "hello" << endl;
                // veto z -> xx decays for signal and z -> mm for background samples (needed for inclusive DYToLL sample)
                ...
                for (Int_t i1 = 0; i1 < muonArr->GetEntriesFast(); i1++)
                {
                    ...
                    if (!isMuonTriggerObj(triggerMenu, tag->hltMatchBits, isData, is13TeV))
                        continue;
                    if (Mu_Pt < tagPt)
                        continue;
                    tagPt = Mu_Pt;
                    ...
                    pfCombIso1 = tag->chHadIso + TMath::Max(tag->neuHadIso + tag->gammaIso - 0.5 * (tag->puIso), Double_t(0));
                    d01 = tag->d0;
                    dz1 = tag->dz;
                ...
                for (Int_t i2 = 0; i2 < muonArr->GetEntriesFast(); i2++)
                {
                    ...
                    if (probept_corr < PT_CUT)
                        continue; // lepton pT cut
                    if (fabs(probe->eta) > ETA_CUT)
                        continue; // lepton |eta| cut

                    double Mu_Pt = probept_corr;

                    if (passID && passMuonID(probe) && Mu_Pt < probePt)
                        continue;
                    if (passID && !passMuonID(probe))
                        continue;
                    if (!passID && !passMuonID(probe) && Mu_Pt < probePt)
                        continue;
                    ...
                    pfCombIso2 = probe->chHadIso + TMath::Max(probe->neuHadIso + probe->gammaIso - 0.5 * (probe->puIso), Double_t(0));
                    d02 = probe->d0;
                    dz2 = probe->dz;
