    ...
    )
{
    ...
    for (UInt_t isam = 0; isam < samplev.size(); isam++) {
        ...
        for (UInt_t ifile = 0; ifile < nfiles; ifile++) {
            ...
            cout << "Processing " << samp->fnamev[ifile] << " [xsec = " << samp->xsecv[ifile] << " pb] ... " << endl;
            cout.flush();
            ...
            for (UInt_t ientry = 0; ientry < 100; ientry++) {
                ...
                Double_t weight = 1;
                if (xsec > 0 && totalWeight > 0)
                    weight = xsec / totalWeight;
                if (hasGen) {
                    genPartArr->Clear();
                ...
                // trigger requirement
                if (!isEleTrigger(triggerMenu, info->triggerBits, isData))
                    continue;
                ...
                for (Int_t i1 = 0; i1 < electronArr->GetEntriesFast(); i1++) {
                    ...
                    // check ECAL gap
                    if (fabs(tag->scEta) >= ECAL_GAP_LOW && fabs(tag->scEta) <= ECAL_GAP_HIGH)
                        continue;
                    ...
                    for (Int_t j = 0; j < muonArr->GetEntriesFast(); j++) {
                        ...
                        // check ECAL gap
                        if (fabs(trkProbe->eta) >= ECAL_GAP_LOW && fabs(trkProbe->eta) <= ECAL_GAP_HIGH)
                            continue;
                        ...
                        if (isSignal && hasGen) {
                            if (tagmatch1)
                                probematch = (glep2_ && toolbox::deltaR(trkProbe->eta, trkProbe->phi, glep2_->Eta(), glep2_->Phi()) < 0.05);
                            else if (tagmatch2)
                                probematch = (glep1_ && toolbox::deltaR(trkProbe->eta, trkProbe->phi, glep1_->Eta(), glep1_->Phi()) < 0.05);

                            if (probematch)
                                hasGenMatchSC = kTRUE;
                        }

                        if (trkProbept_corr < PT_CUT)
                            continue; // Supercluster ET cut ("pt" = corrected by PV position)
                        if (fabs(trkProbe->eta) > ETA_CUT)
                            continue; // Supercluster |eta| cuts
                        // isolation cut
                        if (trkProbe->trkIso > 0.15 * trkProbept_corr)
                            continue;
                        // quality cuts
                        if (trkProbe->nTkLayers < 6 || trkProbe->nPixHits < 1)
                            continue;
...
*/
                        ...
                        if (isSignal && hasGen) {
                            ...
                            Bool_t match1 = (((glep1) && toolbox::deltaR(tag->eta, tag->phi, glep1->Eta(), glep1->Phi()) < 0.3) || ((glep2) && toolbox::deltaR(tag->eta, tag->phi, glep2->Eta(), glep2->Phi()) < 0.3));
