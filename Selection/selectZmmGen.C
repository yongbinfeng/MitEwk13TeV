    ...
    for (UInt_t isam = 0; isam < samplev.size(); isam++) {
        ...
        for (UInt_t ifile = 0; ifile < nfiles; ifile++) {
            ...
            for (UInt_t ientry = 0; ientry < eventTree->GetEntries(); ientry++) {
                ...
                if (xsec > 0 && totalWeightGen > 0)
                    weightGen = xsec / totalWeightGen;
                if (xsec > 0 && totalWeight > 0)
                    weight = xsec / totalWeight;
                if (xsec > 0 && totalWeightUp > 0)
                    weightUp = xsec / totalWeightUp;
                if (xsec > 0 && totalWeightDown > 0)
                    weightDown = xsec / totalWeightDown;
                if (hasGen) {
                    genPartArr->Clear();
                    genBr->GetEntry(ientry);
                ...
                if (isMuonTrigger(triggerMenu, info->triggerBits))
                    passMuTrigger = kTRUE;
                ...
                for (Int_t i1 = 0; i1 < muonArr->GetEntriesFast(); i1++) {
                    ...
                    if (isMuonTriggerObj(triggerMenu, mu->hltMatchBits, kFALSE))
                ...
                if (isSignal && hasGen) {
                    ...
                    Bool_t match1 = (((glep1) && toolbox::deltaR(vlep1.Eta(), vlep1.Phi(), glep1->Eta(), glep1->Phi()) < 0.5) || ((glep2) && toolbox::deltaR(vlep1.Eta(), vlep1.Phi(), glep2->Eta(), glep2->Phi()) < 0.5));

                    Bool_t match2 = (((glep1) && toolbox::deltaR(vlep2.Eta(), vlep2.Phi(), glep1->Eta(), glep1->Phi()) < 0.5) || ((glep2) && toolbox::deltaR(vlep2.Eta(), vlep2.Phi(), glep2->Eta(), glep2->Phi()) < 0.5));
