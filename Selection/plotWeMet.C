    ...
    )
{
    ...
    for (UInt_t ich = 0; ich < 3; ich++) {
        for (UInt_t isam = 0; isam < samplev.size(); isam++) {
            ...
            hMetv[ich].push_back(new TH1D(hname, "", 50, 0, 100));
            hMetv[ich][isam]->Sumw2();
            sprintf(hname, "hMet2_ch%i_%i", ich, isam);
            hMet2v[ich].push_back(new TH1D(hname, "", 50, 0, 100));
            hMet2v[ich][isam]->Sumw2();

            sprintf(hname, "hMvaMet_ch%i_%i", ich, isam);
            hMvaMetv[ich].push_back(new TH1D(hname, "", 50, 0, 100));
            hMvaMetv[ich][isam]->Sumw2();
            sprintf(hname, "hMvaMet2_ch%i_%i", ich, isam);
            hMvaMet2v[ich].push_back(new TH1D(hname, "", 50, 0, 100));
            hMvaMet2v[ich][isam]->Sumw2();

            sprintf(hname, "hTrkMet_ch%i_%i", ich, isam);
            hTrkMetv[ich].push_back(new TH1D(hname, "", 50, 0, 100));
            hTrkMetv[ich][isam]->Sumw2();
            sprintf(hname, "hTrkMet2_ch%i_%i", ich, isam);
            hTrkMet2v[ich].push_back(new TH1D(hname, "", 50, 0, 100));
            hTrkMet2v[ich][isam]->Sumw2();
        }
    }
...
*/
    ...
    for (UInt_t ich = 0; ich < 3; ich++) {
        ...
        if (ich == eInc)
            sprintf(sigtext, "W#rightarrowe#nu");
        else if (ich == ePos)
            sprintf(sigtext, "W^{+}#rightarrowe^{+}#nu");
        else if (ich == eNeg)
            sprintf(sigtext, "W^{-}#rightarrowe^{-}#nu");
        ...
        for (UInt_t isam = 0; isam < samplev.size(); isam++) {
            ...
            if (snamev[isam].CompareTo("we") != 0)
                continue;

            char sigtext_pf[100];
            char sigtext_mva[100];
            char sigtext_trk[100];
        ...
        for (UInt_t isam = 0; isam < samplev.size(); isam++) {
            ...
            if (snamev[isam].CompareTo("top") == 0)
                nttbar_pf += hMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("ww") == 0 || snamev[isam].CompareTo("wz") == 0 || snamev[isam].CompareTo("zz") == 0)
                ndiboson_pf += hMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("wx") == 0)
                nwtaunu_pf += hMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("zxx") == 0)
                ndy_pf += hMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("we") == 0)
                nsig_pf += hMetv[ich][isam]->Integral();
        ...
        for (UInt_t isam = 1; isam < samplev.size(); isam++) {
            // For making yield table
            if (snamev[isam].CompareTo("top") == 0)
                nttbar_mva += hMvaMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("ww") == 0 || snamev[isam].CompareTo("wz") == 0 || snamev[isam].CompareTo("zz") == 0)
                ndiboson_mva += hMvaMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("wx") == 0)
                nwtaunu_mva += hMvaMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("zxx") == 0)
                ndy_mva += hMvaMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("we") == 0)
                nsig_mva += hMvaMetv[ich][isam]->Integral();
            ...
            if (samplev[isam]->label.CompareTo("EWK+t#bar{t}") == 0) {
                if (ewkDrawn == 0) {
                    ...
                    plotMet_mva.AddToStack(hMvaMetv[ich][isam], samplev[isam]->color, samplev[isam]->linecol);
            } else if (samplev[isam]->label.CompareTo("W#rightarrowe#nu") == 0) {
        ...
        for (UInt_t isam = 1; isam < samplev.size(); isam++) {
            ...
            if (samplev[isam]->label.CompareTo("EWK+t#bar{t}") == 0) {
                if (ewkDrawn == 0) {
                    ...
                    plotMet2_mva.AddToStack(hMvaMet2v[ich][isam], samplev[isam]->color, samplev[isam]->linecol);
            } else if (samplev[isam]->label.CompareTo("W#rightarrowe#nu") == 0) {
        ...
        for (UInt_t isam = 1; isam < samplev.size(); isam++) {
            // For making yield table
            if (snamev[isam].CompareTo("top") == 0)
                nttbar_trk += hTrkMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("ww") == 0 || snamev[isam].CompareTo("wz") == 0 || snamev[isam].CompareTo("zz") == 0)
                ndiboson_trk += hTrkMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("wx") == 0)
                nwtaunu_trk += hMvaMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("zxx") == 0)
                ndy_trk += hTrkMetv[ich][isam]->Integral();
            else if (snamev[isam].CompareTo("we") == 0)
                nsig_trk += hTrkMetv[ich][isam]->Integral();
