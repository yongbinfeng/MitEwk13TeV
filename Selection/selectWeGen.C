        ...
        for (UInt_t ifile = 0; ifile < nfiles; ifile++) {
            ...
            for (UInt_t ientry = 0; ientry < eventTree->GetEntries(); ientry++) {
                ...
                for (Int_t i = 0; i < electronArr->GetEntriesFast(); i++) {
                    ...
                    // check ECAL gap
                    if (fabs(ele->scEta) >= ECAL_GAP_LOW && fabs(ele->scEta) <= ECAL_GAP_HIGH)
                        continue;
                    ...
                    if (fabs(ele->scEta) > VETO_ETA)
                        continue; // loose lepton |eta| cut
                    if (elescEt_corr < VETO_PT)
                        continue; // loose lepton pT cut
                    if (passEleLooseID(ele, info->rhoIso))
                    ...
                    if (fabs(ele->scEta) > ETA_CUT)
                        continue; // lepton |eta| cut
                    if (elescEt_corr < PT_CUT)
                        continue; // lepton pT cut
                    if (!passEleID(ele, info->rhoIso))
                        continue; // lepton selection
                    if (!isEleTriggerObj(triggerMenu, ele->hltMatchBits, kFALSE, isData))
                        continue;
                ...
                if (passSel) {
                    ...
                    pfCombIso = goodEle->chHadIso + TMath::Max(goodEle->neuHadIso + goodEle->gammaIso - (info->rhoIso) * getEffAreaEl(goodEle->scEta), 0.);
