    ...
    const Int_t sigma = 0)
{
    ...
    EnergyScaleCorrection_class eleCorr(corrFiles.Data());
    eleCorr.doScale = true;
    eleCorr.doSmearings = true;
    ...
    for (UInt_t isam = 0; isam < samplev.size(); isam++) {
        ...
        for (UInt_t ifile = 0; ifile < nfiles; ifile++) {
            ...
            for (UInt_t ientry = 0; ientry < eventTree->GetEntries(); ientry++) {
                ...
                if (isEleTrigger(triggerMenu, info->triggerBits, 0))
                    passElTrigger = kTRUE;
                ...
                for (Int_t i = 0; i < electronArr->GetEntriesFast(); i++) {
                    ...
                    if (fabs(vEle.Eta()) >= ECAL_GAP_LOW && fabs(vEle.Eta()) <= ECAL_GAP_HIGH)
                        continue; // check ECAL gap
                    if (!passEleID(el, vEle, info->rhoIso))
                        continue; // lepton selection

                    if (isEleTriggerObj(triggerMenu, el->hltMatchBits, kFALSE, 0))
